...
class Carousel {
  ...
  constructor(root) {
    ...
    this.#sourceImages = this.#readImages();
  ...
  #createSlides() {
    this.#buttons.forEach((btn, i) => {
      const image = this.#sourceImages[i] || createImage();
  ...
  #readImages() {
    const raw = this.#root.getAttribute("data-images");
    if (!raw) return [];
    ...
    try {
      ...
      return parsed.filter(
        (item) => item && typeof item.thumb === "string" && typeof item.full === "string",
      );
    } catch {
      return [];
    }
  ...
  #updateDisplayImage(image) {
    ...
    const { full, id, alt } = image;

    if (this.#displayImg.src !== full) {
      this.#displayImg.src = full;
      this.#displayImg.alt = alt || `Image ${id}`;
    }
